<template>
  <div class="calculator-border">
    <div class="calculator-display">
      <div v-cloak class="calculator-formula" :title="formula">{{ formula }}</div>
      <div v-cloak class="calculator-result" :title="result">{{ result }}</div>
    </div>
    ...
    <div class="calculator-items">
      ...
      <div class="calculator-row-6">
        <div class="calculator-item" @click="operateBinary()">二进制</div>
        <div class="calculator-item" @click="operateOctal()">八进制</div>
        <div class="calculator-item" @click="operateDecimal()">十进制</div>
        <div class="calculator-item" @click="operateHex()">十六进制</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    operateBinary() {
      this.equal()
      this.result = (+this.result).toString(2)
    },
    operateOctal() {
      this.equal()
      this.result = (+this.result).toString(8)
    },
    operateDecimal() {
      this.equal()
    ...
    operateHex() {
      this.equal()
      this.result = (+this.result).toString(16)
    }
  }
}
</script>
...
$styleColor1: rgb(237, 23, 31);
$styleColor2: rgb(253, 101, 106);
...
.calculator .calculator-border .calculator-display {
  border: 0.15rem solid $styleColor1;
  ...
  box-shadow: 0.1rem 0.1rem 0.5rem $styleColor1;
}
...
.calculator .calculator-border .calculator-items .calculator-item {
  border: 0.1rem solid $styleColor1;
  width: 16.666%;
  height: 4rem;
...
.calculator .calculator-border .calculator-items .calculator-item:hover {
  // font-weight: 600;
  color: $styleColor1;
  box-shadow: 0.3rem 0.3rem 0.5rem $styleColor1;
}
...
.calculator .calculator-border .calculator-row-6 .calculator-item {
  width: 21.5%;
  font-size: 18px;
  margin-bottom: 2.3%;
...
.calculator .calculator-border .calculator-row-6 .calculator-item:hover {
  color: #fff;
  box-shadow: 0.1rem 0.1rem 0.3rem rgb(85, 65, 65);
}
