export default {
    ...
    matchTableData() {
      this.matchTableData.forEach(rowData => {
        if (this.isAddedExitisData(rowData)) {
          this.$set(rowData, 'state', true)
        } else {
          this.$set(rowData, 'state', false)
        }
      })
    },
    settingData() {
  ...
  methods: {
    ...
    handleMatchAddClick(rowData) {
      ...
      this.changeRowData(rowData, true)
    ...
    handleAddedDeleteClick(rowData) {
      ...
      /* 取出匹配的和不匹配的 */
      const data = this.addedTableData.filter(data => data.id === rowData.id)
      this.addedTableData = this.addedTableData.filter(data => data.id !== rowData.id)
